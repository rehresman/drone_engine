	...
	*new { arg context, doneCallback;
	...
	alloc {
		...
		SynthDef(\oscillator, {|freqMaster, freqIndividual, spread, grungeAmt, crossInBus, outBus, fmOutBus, fmInBus, flip, lfoAmt, ampInBus|
			...
			spread = spread * ((-1)**flip) /2;
			sig = SinOsc.ar([freq+spread,freq-spread]);
			sig = sig * amp;
		...
		SynthDef(\mixer, {|in1Bus, in2Bus, in1Level, in2Level, outBus|
			...
			sig = (in1 * Lag.kr(in1Level,0.18)) + (in2 * Lag.kr(in2Level,0.18));
		...
		SynthDef(\dualLFO, {|rate1, rate2, outBus|
			...
			sig = [VarSaw.ar(Lag.kr(rate1,0.18)*(1+instability)), VarSaw.ar(Lag.kr(rate2,0.18)*(1-instability))];
			Out.ar(outBus, sig);
		}).add;
